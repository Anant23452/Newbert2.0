...
const CORE_CS = new Set(["dbms", "oop", "operating-systems", "computer-networks"]);
...
function taskBlueprint(skillId, skillName, hasEvidence) {
  const subject = skillName;
  if (skillId === "dbms") return [
  ...
  if (skillId === "oop") return [["learn", "OOP foundations", "Review encapsulation, abstraction, inheritance, and polymorphism.", 60], ["practice", "Model a small domain", "Turn requirements into classes, responsibilities, and relationships.", 75], ["review", "SOLID and trade-offs", "Identify when common OOP patterns help or add unnecessary complexity.", 60], ["interview", "Practice 15 OOP interview questions", "Explain design choices with concrete code examples.", 80], ["assessment", "OOP verification assessment", "Submit an assessment result when Newbert assessment verification is available.", 45]];
  if (CORE_CS.has(skillId)) return [["learn", `${subject} foundations`, `Learn the core concepts and vocabulary behind ${subject}.`, 75], ["learn", `${subject} mechanisms`, "Connect the concepts to realistic system behaviour and trade-offs.", 75], ["practice", `Solve ${subject} scenarios`, "Work through practical questions before checking explanations.", 75], ["interview", `Practice ${subject} interview questions`, "Explain your answers aloud with examples.", 75], ["assessment", `${subject} verification assessment`, "Submit an assessment result when Newbert assessment verification is available.", 45]];
  if (skillId === "react") return [["learn", "Component fundamentals", "Build reusable components with clear props and state boundaries.", 70], ["learn", "Hooks and data flow", "Use effects, derived state, and event handling intentionally.", 75], ["build", "Integrate one API-backed feature", "Build loading, error, and empty states around a real API call.", 100], ["project", "Publish a small React feature", "Add the feature to a reviewable repository or project.", 120], ["github", "Verify project evidence", "Sync GitHub after the implementation is available.", 25]];
  if (["nodejs", "express", "rest-api"].includes(skillId)) return [["learn", `${subject} fundamentals`, "Review the runtime, request lifecycle, and modular application structure.", 70], ["build", "Build one API endpoint", "Implement validation, errors, and a clear response contract.", 100], ["practice", "Handle failure cases", "Test missing input, invalid data, and authorization boundaries.", 60], ["project", "Publish backend evidence", "Add the implementation to a reviewable project.", 90], ["github", "Verify project evidence", "Sync GitHub after the implementation is available.", 25]];
  if (skillId === "dsa") return [["learn", "Review core patterns", "Choose patterns based on constraints, not memorized answers.", 60], ["leetcode", "Solve a focused problem set", "Practice a balanced set and use accepted solutions as evidence.", 120], ["review", "Review mistakes", "Write down why each missed solution failed.", 45], ["interview", "Explain solutions aloud", "Practice complexity analysis and edge cases.", 60]];
  const firstTitle = hasEvidence ? `Strengthen practical ${subject}` : `Learn ${subject} foundations`;
  return [["learn", firstTitle, hasEvidence ? "Skip repeated basics and focus on the next useful concepts." : `Build a reliable foundation in ${subject}.`, 75], ["practice", `Practice ${subject} scenarios`, "Apply the concept to small, realistic exercises.", 75], ["build", `Use ${subject} in a focused feature`, "Create reviewable work instead of only consuming material.", 100], ["review", `Review ${subject} trade-offs`, "Explain what you chose and why.", 45], ["github", "Sync or submit real evidence", "Connect an existing supported source when it can verify this skill.", 25]];
}

function buildTasks(skillId, skillName, hasEvidence) {
  return taskBlueprint(skillId, skillName, hasEvidence).map(([type, title, description, minutes], index) => task(skillId, index + 1, type, title, description, minutes));
}
...
function generateImprovementPlan({ profile, skill, alumni = [], existingPlan = null }) {
  ...
  const tasks = existingPlan?.tasks?.length ? existingPlan.tasks : buildTasks(skillId, skillName, hasEvidence);
