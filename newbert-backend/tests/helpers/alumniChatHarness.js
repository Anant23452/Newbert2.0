...
function setup(){
 const sessions=new Map(),alumni=new Map(),documents=new Map();let aiMode='failure';
 ...
 mock.method(Alumni,'findOne',q=>({lean:async()=>[...alumni.values()].find(a=>q.userId?String(a.userId)===String(q.userId):String(a._id)===String(q._id))||null}));
 mock.method(Alumni,'findOneAndUpdate',async(q,update)=>{const existing=alumni.get(String(q.userId));const item={...existing,...clone(update.$set),userId:q.userId,_id:existing?._id||'507f1f77bcf86cd799439031',createdAt:existing?.createdAt||new Date()};await new Alumni(item).validate();alumni.set(String(q.userId),item);return item;});
 mock.method(Alumni,'updateOne',async(q,update)=>{const a=alumni.get(String(q.userId));if(a)Object.assign(a,clone(update.$set));return {modifiedCount:a?1:0};});
 ...
 const app=express();app.use((req,res,next)=>{res.set('Access-Control-Allow-Origin','http://127.0.0.1:5184');res.set('Access-Control-Allow-Headers','Content-Type,Authorization');res.set('Access-Control-Allow-Methods','GET,POST,PATCH,OPTIONS');if(req.method==='OPTIONS')return res.sendStatus(204);next();});app.use(express.json({limit:'3mb'}));app.use('/api/alumni-chat',require('../../routes/alumniChatRoutes'));
 ...
 return {app,sessions,alumni,documents,college,userId,otherId,token:id=>jwt.sign({id:id||userId},process.env.JWT_SECRET,{expiresIn:'2h'}),aiMode:value=>{aiMode=value;}};
}
