...
test("placement alumni comparison uses submitted DSA and project values", () => {
  const alumni = dummyAlumniFixtures.find((item) => item.dummyKey === "sneha-singh-rec-2025");
  ...
  assert.equal(result.dimensions.find((item) => item.key === "dsa").alumni.value, 225);
  assert.ok(result.differences.some((item) => item.includes("225")));
...
test("GATE comparison uses only recorded student stage evidence", () => {
  const alumni = dummyAlumniFixtures.find((item) => item.dummyKey === "aditya-tiwari-rec-2025");
  ...
  assert.equal(result.dimensions.find((item) => item.key === "tests").alumni.value, 35);
  assert.ok(result.differences.some((item) => item.includes("35 tests")));
});
...
test("course reviews retain rating, recommendation, and path", () => { const course = dummyAlumniFixtures.find((item) => item.dummyKey === "aditya-tiwari-rec-2025").courses[0]; assert.equal(course.path, "gate"); assert.equal(course.rating, 4.6); assert.equal(course.wouldRecommend, true); assert.ok(course.review); });
