    ...
    <header className="site-nav sticky top-0 z-50 border-b border-slate-200 bg-[#f7f8fa]/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-5">
        <Link to="/" className="flex items-center gap-2.5" aria-label="Newbert home">
          <span className="grid h-8 w-8 place-items-center rounded-md bg-teal-700 font-bold text-white">
            <img className="cover h-full w-full" src="/src/assets/2-removebg-preview(1).png" alt="" />
          </span>
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          {user?.isAdmin && <button onClick={() => navigate("/admin/courses")} className="text-xs font-extrabold text-orange-700 hover:text-orange-900">Admin Courses</button>}
      ...
      {open && <nav id="primary-navigation" className="site-nav-menu border-t border-slate-200 bg-white px-5 py-3">
        <div className="mx-auto flex max-w-6xl flex-col gap-1">
          ...
          {user?.isAdmin && <button onClick={() => { setOpen(false); navigate("/admin/courses"); }} className="rounded-md border border-orange-400 px-3 py-2 text-left text-sm font-bold text-orange-700">Admin Courses</button>}
