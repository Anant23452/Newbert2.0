...
export default function TodayMomentum({ profile }) {
  ...
  const totals = days.reduce((sum, item) => ({ github: sum.github + item.github, leetcode: sum.leetcode + item.leetcode }), { github: 0, leetcode: 0 });
  ...
  return <section className="today-momentum" aria-label="Your activity week"><div className="today-section-heading"><h2><TrendingUp size={19}/>Your rhythm</h2><Link to="/profile">History <ArrowRight size={15}/></Link></div><div className="today-rhythm-heading"><strong>{active}<span>/ 7 days</span></strong><p>{active >= 5 ? "You kept showing up this week." : active > 0 ? "You have a starting point. Build on it." : "Your next small step can start here."}</p></div>
    <div className="today-week-legend"><span><i/>{totals.github} commits</span><span><i/>{totals.leetcode} accepted*</span></div>
    <div className="today-week" role="group" aria-label="Activity in the last seven days">{days.map((item) => <button key={item.key} onClick={() => setSelected(item.key)} aria-pressed={day.key === item.key} aria-label={`${item.dateLabel}: ${item.github} GitHub commits, ${item.leetcode} LeetCode accepted records`}><span className="today-bar-track"><span className={item.total ? "today-bar active" : "today-bar"} style={{ height: `${Math.max(6, item.total / max * 100)}%` }}>{item.total > 0 && <><span className="today-bar-github" style={{ height: `${item.github / item.total * 100}%` }}/><span className="today-bar-leetcode" style={{ height: `${item.leetcode / item.total * 100}%` }}/></>}</span></span><span>{item.label}</span><small>{item.dateLabel.split(" ")[0]}</small></button>)}</div>
