import AlumniStoryDetails from "../components/AlumniChat/AlumniStoryDetails";
...
function pathsOf(alumni) { if (alumni.careerPaths?.length) return alumni.careerPaths; return [(["gate", "psu"].includes(alumni.path || alumni.outcomeType) ? "gate" : (alumni.path || alumni.outcomeType)==="other" ? "other" : "placement")]; }
...
export default function AlumniIntelligenceProfile() {
  ...
  const paths = pathsOf(alumni); const firstName = (alumni.name || "Alumni").split(" ")[0];
  return <main className="mx-auto max-w-6xl px-5 py-10 md:py-14"><div className="flex items-center justify-between"><button onClick={() => navigate("/alumni-wall")} className="text-sm font-bold text-orange-700">← Back to alumni outcomes</button><Link to={`/alumni-wall/${alumniId}/career-dna`} className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500/10 border border-orange-500/30 px-3.5 py-1.5 text-xs font-bold text-orange-700 hover:bg-orange-500/20 transition-colors">🧬 View Career DNA</Link></div><ProfileHeader alumni={alumni} paths={paths}/>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]"><div className="space-y-6">{alumni.story ? <AlumniStoryDetails story={alumni.story} verified={alumni.verified}/> : <><Outcome alumni={alumni} path={activePath}/>{activePath === "placement" ? <PlacementPreparation alumni={alumni}/> : <GatePreparation alumni={alumni}/>}<Journey alumni={alumni} path={activePath}/><Courses courses={(alumni.courses || []).filter((course) => course.path === activePath)}/><Advice alumni={alumni}/></>}<Comparison state={comparison}/></div>
      <aside><section className="surface rounded-2xl border border-slate-200 p-5 lg:sticky lg:top-24"><p className="text-xs font-extrabold uppercase tracking-widest text-orange-600">Compare with me</p><h2 className="mt-2 text-xl font-black text-slate-950">How is your path different?</h2><p className="mt-2 text-sm leading-6 text-slate-600">{activePath === "other" ? `This journey is available to learn from. Skill comparison currently supports placement and GATE journeys.` : `Newbert compares only available evidence for you and ${firstName}. It is not an outcome probability.`}</p><button onClick={compare} disabled={comparison.loading || activePath === "other"} className="mt-5 w-full rounded-lg bg-orange-500 px-4 py-3 text-sm font-extrabold text-slate-950 disabled:opacity-50">{comparison.loading ? "Comparing..." : "Compare with me"}</button>{isAuthenticated && activePath !== "other" && <button onClick={addToRoadmap} className="mt-3 w-full rounded-lg border border-slate-300 px-4 py-3 text-sm font-extrabold text-slate-700">Use this path in my roadmap</button>}{roadmapStatus && <p className="mt-3 text-xs font-bold text-slate-500">{roadmapStatus}</p>}
