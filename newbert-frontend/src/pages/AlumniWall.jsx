...
export default function AlumniWall() {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-12 md:py-16">
    <div className="max-w-2xl"><p className="eyebrow">Alumni journeys</p><h1 className="mt-3 text-3xl font-extrabold tracking-tight text-slate-950 md:text-4xl">Learn from students who have already made it through.</h1><p className="mt-4 text-sm leading-6 text-slate-600">Filter alumni outcomes by path, college, company or role. These are the profiles Newbert uses to make your plan relevant.</p><Link to="/alumni/onboarding" className="mt-5 inline-flex rounded-lg bg-orange-500 px-5 py-3 text-sm font-extrabold text-[#171918] hover:bg-orange-400">Share Your Journey →</Link></div>
    ...
    {loadError && <p className="mt-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900">{loadError} Showing available public stories instead.</p>}
...
function VerificationDialog({ onClose }) { return <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/45 p-5" role="dialog" aria-modal="true" aria-label="Outcome verification"><section className="w-full max-w-md rounded-lg bg-white p-6 shadow-2xl"><p className="eyebrow">Outcome verification</p><h2 className="mt-2 text-xl font-extrabold text-slate-950">Built around real student journeys</h2><p className="mt-4 text-sm leading-6 text-slate-600">Newbert records the student’s college, batch, role, company, preparation pattern, and supporting context before an outcome is shown. Published stories are self-reported unless a verified badge is shown. Private evidence can be submitted for review; uploading it does not automatically verify a claim.</p><button onClick={onClose} className="mt-6 bg-orange-500 px-4 py-2.5 text-sm font-extrabold text-[#171918] hover:bg-orange-400">Got it</button></section></div>; }
