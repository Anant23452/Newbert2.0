...
export default function ReadinessAnalysis({ analysis, loading, error, onEdit }) {
  ...
  const evidence = analysis.evidenceReadiness;
  const categories = Object.entries(analysis.coverage).filter(([key]) => key !== "overall");
  return <>
    {evidence && <section className="surface rounded-2xl border border-orange-200 bg-white p-6 shadow-sm md:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-widest text-orange-600">Evidence-backed estimate</p><h2 className="mt-2 text-2xl font-extrabold text-slate-950">Placement readiness</h2><p className="mt-2 text-sm text-slate-600">Available evidence supports an estimated {evidence.overallReadiness ?? "unavailable"}% readiness.</p></div><span className={`rounded-md border px-2.5 py-1 text-xs font-extrabold uppercase ${confidenceTone[evidence.confidenceLabel.toLowerCase()]}`}>Evidence confidence: {evidence.confidenceLabel}</span></div>
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Object.entries(evidence.dimensions).map(([key, item]) => <div key={key} className="rounded-lg border border-slate-200 p-4"><p className="text-xs font-extrabold uppercase text-slate-500">{key}</p><p className="mt-1 text-xl font-black text-slate-950">{item.score == null ? "Unavailable" : `${item.score}%`}</p><p className="mt-1 text-xs text-slate-500">Confidence {Math.round(item.confidence * 100)}%</p></div>)}</div>
      {evidence.skillEvidence?.length > 0 && <details className="mt-6 border-t border-slate-100 pt-5"><summary className="cursor-pointer text-sm font-extrabold text-orange-700">View skill evidence</summary><div className="mt-4 grid gap-3 sm:grid-cols-2">{evidence.skillEvidence.slice(0, 8).map((skill) => <div key={skill.normalizedSkill} className="rounded-lg border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><p className="font-extrabold text-slate-950">{skill.skill}</p><span className="text-xs font-bold capitalize text-orange-700">{skill.level.replaceAll("_", " ")}</span></div><p className="mt-1 text-sm text-slate-600">Evidence score {skill.score} · Confidence {Math.round(skill.confidence * 100)}%</p><ul className="mt-3 space-y-1 text-xs leading-5 text-slate-500">{skill.sources.slice(0, 4).map((source, index) => <li key={`${source.type}-${index}`}>✓ {source.evidence}</li>)}</ul></div>)}</div></details>}
      {evidence.limitations?.length > 0 && <div className="mt-5 rounded-lg bg-amber-50 p-4 text-sm leading-6 text-amber-900">{evidence.limitations.map((item) => <p key={typeof item === "string" ? item : item.message}>{typeof item === "string" ? item : item.message}</p>)}</div>}
      <p className="mt-4 text-xs font-semibold text-slate-500">{evidence.disclaimer}</p>
    </section>}
