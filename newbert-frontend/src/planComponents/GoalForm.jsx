...
export default function GoalForm({ profile, existingPlan, submitting, error, onCancel, onSubmit }) {
  ...
  useEffect(() => { let active = true; getRoadmapTargetJobs().then((jobs) => { if (active) setTargetJobs(jobs); }).catch(() => { if (active) setTargetJobs([]); }).finally(() => { if (active) setJobsLoading(false); }); return () => { active = false; }; }, []);
  ...
  const toggleJob = (job) => { setPreview(null); setTarget((current) => { const selected = current.jobIds.includes(job.id) ? current.jobIds.filter((id) => id !== job.id) : [...current.jobIds, job.id].slice(0, 5); return { ...current, jobIds: selected, role: current.role || job.title, company: selected.length === 1 ? job.company : current.company }; }); };
  ...
  return <main className="min-h-screen bg-[#111827] px-5 py-12 text-white md:py-16"><div className="mx-auto max-w-5xl"><p className="text-xs font-extrabold uppercase tracking-[.2em] text-orange-400">Build my plan</p><h1 className="mt-3 text-3xl font-black md:text-5xl">Choose the goal you are preparing for.</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300">Newbert already loaded your saved MongoDB profile. Only update the goal information that should shape this plan.</p>
    <section className="mt-8 grid gap-5 rounded-2xl border border-white/10 bg-white/5 p-5 md:grid-cols-[.75fr_1.25fr] md:p-7"><aside className="rounded-xl border border-white/10 bg-black/20 p-5"><p className="text-xs font-extrabold uppercase tracking-widest text-orange-300">Profile used</p><h2 className="mt-3 text-xl font-extrabold">{profile.name}</h2><p className="mt-1 text-sm text-slate-300">{profile.college} · {profile.branch}</p><div className="mt-5 flex flex-wrap gap-2">{skills.length ? skills.slice(0, 10).map((skill) => <span key={skill} className="rounded-full border border-white/10 px-2.5 py-1 text-xs font-bold text-slate-200">{skill}</span>) : <span className="text-xs text-slate-400">No saved skills yet</span>}</div><div className="mt-5 space-y-1 text-xs font-semibold text-slate-400"><p>GitHub: {profile.connections?.github?.connected ? "Connected" : "Not connected"}</p><p>LeetCode: {profile.connections?.leetcode?.connected ? "Connected" : "Not connected"}</p><p>Projects: {profile.projects ?? "Not added"}</p><p>CGPA: {profile.cgpa ?? "Not added"}</p></div></aside>
      <form onSubmit={(event) => { event.preventDefault(); if (preview) onSubmit({ ...target, weeklyHours: Number(target.weeklyHours), understoodCurrentStage: preview.understoodCurrentStage }); else requestPreview(); }}><p className="text-sm font-extrabold">Roadmap target</p><div className="mt-3 inline-flex rounded-lg border border-white/15 p-1"><button type="button" onClick={() => { update("mode", "role"); setPreview(null); }} className={`rounded-md px-4 py-2 text-sm font-bold ${target.mode === "role" ? "bg-orange-500 text-slate-950" : "text-slate-300"}`}>Role Roadmap</button><button type="button" onClick={() => { update("mode", "job"); setPreview(null); }} className={`rounded-md px-4 py-2 text-sm font-bold ${target.mode === "job" ? "bg-orange-500 text-slate-950" : "text-slate-300"}`}>Job Roadmap</button></div>{target.mode === "job" && <section className="mt-5 rounded-lg border border-white/10 bg-black/10 p-4"><p className="text-sm font-extrabold">Select up to five saved jobs</p><p className="mt-1 text-xs text-slate-400">Requirements shared by multiple selected jobs receive higher deterministic priority.</p><div className="mt-3 grid gap-2">{jobsLoading ? <p className="text-sm text-slate-400">Loading saved jobs...</p> : targetJobs.length ? targetJobs.map((job) => <label key={job.id} className="flex cursor-pointer items-start gap-3 rounded-lg border border-white/10 p-3"><input type="checkbox" checked={target.jobIds.includes(job.id)} onChange={() => toggleJob(job)} className="mt-1 size-4 accent-orange-500"/><span><span className="block text-sm font-extrabold">{job.title}</span><span className="mt-1 block text-xs text-slate-400">{job.company} · {job.requirementCount} structured requirements</span></span></label>) : <p className="text-sm text-slate-400">Save a verified job from the Jobs Portal before building a Job Roadmap.</p>}</div></section>}<p className="mt-6 text-sm font-extrabold">What are you preparing for?</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{GOALS.map(([value, label]) => <button type="button" key={value} onClick={() => chooseGoal(value)} className={`rounded-lg border px-3 py-3 text-left text-sm font-bold transition ${target.type === value ? "border-orange-400 bg-orange-400 text-slate-950" : "border-white/15 bg-white/5 text-slate-200 hover:border-orange-300"}`}>{label}</button>)}</div>
        ...
        <div className="mt-7 flex flex-col gap-3 sm:flex-row"><button disabled={submitting || previewing || !target.role.trim() || (target.mode === "job" && !target.jobIds.length)} className="rounded-lg bg-orange-500 px-5 py-3 text-sm font-extrabold text-slate-950 hover:bg-orange-400 disabled:opacity-50">{submitting ? "Building your plan..." : previewing ? "Understanding your context..." : preview ? "Build this roadmap" : "Review roadmap inputs"}</button>{onCancel && <button type="button" onClick={onCancel} className="rounded-lg border border-white/15 px-5 py-3 text-sm font-bold text-slate-200">Keep current plan</button>}</div>
